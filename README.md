...
3. Run the setup script. It will:
  a. Delete the default `~/.config/karabiner` folder
  b. Create a symlink between the current working directory and `~/.config/karabiner` with `ln -s "$PWD" ~/.config`
  c. [Restart karabiner_console_user_server](https://karabiner-elements.pqrs.org/docs/manual/misc/configuration-file-path/) with `` launchctl kickstart -k gui/`id -u`/org.pqrs.karabiner.karabiner_console_user_server ``
